		...
		if ( have_posts() ) {
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class . $item_class ); ?>>
				...
				<?php } ?>
					...
					echo '<div class="post-content">';
					$my_key1 = get_post_meta($post->ID, 'episode_number', true) ? get_post_meta($post->ID, 'episode_number', true) : "";
if($my_key1 != "") { 
	echo '<div class="mykey1">'. $my_key1 . '</div>'; 
					...
					echo et_core_esc_previously( $more ); 
					echo '</div>';
                    $positionName = get_post_meta($post->ID, 'position', true) ? get_post_meta($post->ID, 'position', true) : "";
                    $companyName = get_post_meta($post->ID, 'company', true) ? get_post_meta($post->ID, 'company', true) : "";
                    $authorName = get_the_title();
                    if($positionName!= "" && $companyName != "") { 
                    echo '<p class="testimonial-details">'. $authorName . ' | ' . $positionName . '</br>' . $companyName . '</p>'; 
                    }
					?>
			<?php } // 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote', 'gallery' ?>
